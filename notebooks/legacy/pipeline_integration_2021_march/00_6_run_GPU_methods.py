...
if method == 'scvi':
    ...
    if not np.isnan(n_epochs):
        ...
        integrated, trainer = scIB.integration.runScvi(adata, batch)
elif method == 'scgen':
    print('scgen')
    from tensorflow.python.client import device_lib
    devices = device_lib.list_local_devices()
    ...
    # scvi
    if not np.isnan(n_epochs):
        integrated = scIB.integration.runScanvi(adata, batch, cell_type_key, n_epochs_scVI=n_epochs, n_epochs_scANVI=10, n_layers=int(n_layers), n_hidden=int(n_hidden))
    else:
        integrated = scIB.integration.runScanvi(adata, batch, cell_type_key, n_epochs_scANVI=10, n_layers=int(n_layers), n_hidden=int(n_hidden))
