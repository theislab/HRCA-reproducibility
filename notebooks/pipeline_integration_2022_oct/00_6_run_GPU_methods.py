...
n_epochs = int(n_epochs) if not np.isnan(n_epochs) else n_epochs

print('location of scIB.integration')
print(scIB.integration)
# adata = scIB.integration.runScGen(adata, batch, labels)
if method == 'scvi':
    print('scVI...')
    if not np.isnan(n_epochs):
        integrated = scIB.integration.runScvi(adata, batch, n_epochs=n_epochs)
    else:
    ...
    if not np.isnan(n_epochs):
        ...
        integrated = scIB.integration.runScGen_v2_0_0(adata, batch, cell_type_key, device=device_gpu, verbose=1)
elif method == 'scanvi':
    print('scanvi')
    # scvi
    if not np.isnan(n_epochs):
        integrated = scIB.integration.runScanvi(adata, batch, cell_type_key, n_epochs=n_epochs, n_epochs_scANVI=10, n_layers=int(n_layers), n_hidden=int(n_hidden))
    else:
        integrated = scIB.integration.runScanvi(adata, batch, cell_type_key, n_epochs=10, n_layers=int(n_layers), n_hidden=int(n_hidden))
